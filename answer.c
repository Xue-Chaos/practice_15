    ...
    if(P1IFG &0X04) //按键SW1按下时
    {
        ...
        */
        ...
        if(SW1==0)//确实是SW1按钮触发了外部中断
        {
            ...
            {
                sys_state=1;//布防
                uart_tx_string("SYSTEM RUN\r\n");//每按一次按键 发送一次信息到串口
            }
            else
            {
                sys_state=0;//撤防
                uart_tx_string("SYSTEM STOP\r\n");//每按一次按键 发送一次信息到串口
            }
...
void RunMode(void)
{
    ...
    if(SW2==0)
    {
        alarmDelay=1000; //按下SW2后 设置10秒延时，用来设置跑马灯效果保持约10秒的时间
    }
    
    /*
    在布防模式时，D5常亮    
    在SW2按下后，
    触发报警，报警时 D3 D4 D6轮流点亮，为跑马灯效果，每个灯约500ms亮,
